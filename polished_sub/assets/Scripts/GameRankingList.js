        ...
        if (CC_WECHATGAME) {
            window.wx.getUserCloudStorage({
                ...
                success: function (getres) {
                    console.log('submitScore getUserCloudStorage', 'success', getres);
        ...
        if (CC_WECHATGAME) {
            wx.getUserInfo({
                ...
                success: (userRes) => {
                    ...
                    console.log('fetchFriendData success', userRes.data)
                    let userData = userRes.data[0];
                    //取出所有好友数据
                    wx.getFriendCloudStorage({
                        keyList: ["x" + MAIN_MENU_NUM],
                        success: res => {
                            console.log("fetchFriendData wx.getFriendCloudStorage success", res);
                            let data = res.data;
                            data.sort((a, b) => {
    ...
    gameOverRank(MAIN_MENU_NUM) {
        ...
        if (CC_WECHATGAME) {
            wx.getUserInfo({
                openIdList: ['selfOpenId'],
                success: (userRes) => {
                    console.log('gameOverRank success', userRes.data)
                    let userData = userRes.data[0];
                    //取出所有好友数据
                    wx.getFriendCloudStorage({
                        keyList: ["x" + MAIN_MENU_NUM],
                        success: res => {
                            console.log("gameOverRank wx.getFriendCloudStorage success", res);
                            this.loadingLabel.active = false;
                            let data = res.data;
                            data.sort((a, b) => {
                                if (a.KVDataList.length == 0 && b.KVDataList.length == 0) {
